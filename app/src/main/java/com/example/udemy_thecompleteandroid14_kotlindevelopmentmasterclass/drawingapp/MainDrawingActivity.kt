...
class MainDrawingActivity : AppCompatActivity() {
    ...
    private val requestPermission:ActivityResultLauncher<Array<String>> = registerForActivityResult(ActivityResultContracts.RequestMultiplePermissions()){
        ...
        permissions.entries.forEach{
            ...
            if (isGranted){
                Toast.makeText(this@MainDrawingActivity, "Permission granted for read storage files", Toast.LENGTH_LONG).show()
                val pickIntent = Intent(Intent.ACTION_PICK, MediaStore.Images.Media.EXTERNAL_CONTENT_URI)
                openGalleryLauncher.launch(pickIntent)

            }else{
                if (permissionName == Manifest.permission.READ_EXTERNAL_STORAGE){
                    Toast.makeText(this@MainDrawingActivity, "Oops you just denied the permission", Toast.LENGTH_LONG).show()
                }
            }
        }
    }
                ...
                try {
                    ...
                    runOnUiThread{
                        ...
                        if (result.isNotEmpty()){
                            Toast.makeText(this@MainDrawingActivity, "File saved successfully: $result", Toast.LENGTH_SHORT).show()
                            shareImage(result)
                        }else{
                            Toast.makeText(this@MainDrawingActivity, "Something went wrong while saving the file", Toast.LENGTH_SHORT).show()
                        }
                    }
                }catch (e:Exception){
                    result = ""
